...
class Pokemon():
    ...
    def attack(self, pokemon):
        ...
        while self.hit_points > 0 and pokemon.hit_points > 0:
            chance_self=random.choice([True if n < self.master.crit_chance*1000 else False for n in range(1000)])
            chance_pokemon=random.choice([True if n < pokemon.master.crit_chance*1000 else False for n in range(1000)])
        ...
        # every round they attack eachother
        # Attacker Start the Battle
        # defender hp = defender hp - attacking power of the attacker
...
Types:\t\t{" ".join(self.types)}
        ...
        # reduce(lambda x,y: x+ " " +y, self.types)